...
7. SQLite上の統合済みデータから主要テーブルを再生成した完全版SQL、基準DBへ単独適用できる統合差分SQL、JSONレポートを出力
...
v0.2.0以降の完全版SQLは、A側SQLへB側差分を末尾追加する方式ではありません。投稿・メタ・ターム・対応プラグイン行をSQLite上で統合した後、各テーブルの確定データとして複数行INSERTで再生成します。v0.2.1ではブラウザーインポートの処理時間を抑えるため、INSERTとトランザクションの分割単位を最適化しています。

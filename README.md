...
基準DBと追加側DBの `options` テーブルから `home` / `siteurl` を検出します。比較画面ではURL・ホスト名の置換候補をテーブル別に表示し、チェックしたテーブルだけを置換できます。メールアドレスは追加側ドメインを残さないよう候補をすべて初期選択し、候補ごとの確認リストから残したいものだけ選択を外せます。完全版SQLと統合差分SQLでは、追加側ホストの `http://`、`https://`、www有無のURL、単独のホスト名、選択したメールアドレスのドメインを基準DBの `home`（なければ `siteurl`）と同じホストへ統一します。URLのパス、クエリ、フラグメントと、メールアドレスのローカル部は維持します。

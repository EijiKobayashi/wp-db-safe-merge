...
<section class="page-title">
  <div><div class="eyebrow">STEP 02 — REVIEW</div><h1>比較結果を確認</h1><p>候補を確認し、必要な項目だけ採用してください。</p>
    ...
    <div class="prefix-summary"><span><b>基準DB（SQL <?= $e($baseSideLabel) ?>）</b><small>DB名 <code><?= $e($state['base']['database_name'] ?? '記載なし') ?></code></small><small>テーブル接頭辞 <code><?= $e($state['base']['prefix']) ?></code></small></span><span class="material-symbols-outlined" aria-hidden="true">compare_arrows</span><span><b>追加側（SQL <?= $e($incomingSideLabel) ?>）</b><small>DB名 <code><?= $e($state['incoming']['database_name'] ?? '記載なし') ?></code></small><small>テーブル接頭辞 <code><?= $e($state['incoming']['prefix']) ?></code></small></span></div>
  </div>
    ...
    <?php else: ?>
      ...
      <?php if ($emailCandidates !== []): ?>
        ...
        <section class="email-review-list" data-email-settings data-email-state-key="<?= $e($emailStateKey) ?>"><header><div><h3>Stgメールの変換先ドメインを指定</h3><p>変換する候補だけチェックし、@より後ろのドメインを入力してください。ローカル部は変更しません。入力内容はページ移動後も保持されます。</p></div><b><?= $e(count($emailCandidates)) ?>種類</b></header>
          <div class="email-bulk-controls"><label><span>共通の変換先ドメイン</span><input type="text" placeholder="例: example.com" inputmode="url" autocomplete="off" data-email-bulk-target></label><button class="button secondary small" type="button" data-email-bulk-apply>全候補に設定してチェック</button><button class="text-button" type="button" data-email-clear>保存したメール設定を解除</button><small>入力内容は現在の作業内だけで保持します。個別に異なるドメインが必要な候補は、適用後に各行で変更できます。</small></div>
